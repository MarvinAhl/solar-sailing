...
# Expands a search tree node in beam search, called in parallel
def expand_node(node_zip, mu_sun, au, Am, I0, c, t_search, Nt, N_control, beam_width, tf, neas, ss, search_tree_len):
    open_idx = node_zip[0]
    node = node_zip[1]

    #print("Expanding open list node {}".format(open_idx + 1), file=log_file, flush=True)
    ...
    for ca in cas_arr_i:
        ...
        if sol_fi < 1e-4:
            ...
            t2i = t0i + sol_tofi
            if t2i < tf:
                parent_id_i = search_tree_len + open_idx  # Computes where parent will be inserted in search_tree
...
if __name__ == '__main__':
    ...
    # Number of CPU's used for parallelization
    N_parallel = 6
    ...
    with open("log.txt", 'a') as log_file:
        ...
        # Start parallel
        print("Creating parallel pool ...", file=log_file)
        with Pool(N_parallel) as parpool:
            print("Parallel pool created successfully", file=log_file)
            ...
            while len(open_list) > 0:
                ...
                print("Nodes in open list: {}".format(len(open_list)), file=log_file)

                # Creating the function that is given to the parallel pool
                expand_node_partial = partial(expand_node, mu_sun=mu_sun, au=au, Am=Am, I0=I0, c=c, t_search=t_search,
                                              Nt=Nt, N_control=N_control, beam_width=beam_width, tf=tf, neas=neas, ss=ss, search_tree_len=len(search_tree))

                # Parallel pool inputs
                poollist = list(zip(list(range(len(open_list))), open_list))

                # Expand nodes in parallel
                print("Expanding nodes ...", file=log_file)
                candidate_lists = parpool.map(expand_node_partial, poollist)
                candidate_list = list(chain(*candidate_lists))

                print("Successfully expanded nodes", file=log_file)
                print("{} candidate asteroids found".format(len(candidate_list)), file=log_file)
