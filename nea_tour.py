...
if __name__ == '__main__':
    ...
    while len(open_list) > 0:
        tree_level += 1
        print("Starting tree level {}".format(tree_level))
        
        for open_idx, node in enumerate(open_list):
            print("Expanding open list node {} / {} (tree level {})".format(open_idx + 1, len(open_list), tree_level))
            ...
            print("Initial number of close approache indexes: {}".format(N_ca_i))
            print("(Avg grid triggers per idx: {})".format(grid_cas_points_tot_i / np.max([N_ca_i, 1])))
            ...
            print("Remaining number of close approache indexes: {}".format(np.shape(cas_i)[0]))
            ...
            for ca in cas_arr_i:
                ...
                if ca_et > tf + et_margin:
                    print("Optimization stop: Close approaches after final time")
                ...
                if len(candidate_list) >= beam_width and candidate_list[beam_width-1][1] + et_margin < ca_et:
                    print("Optimization stop: Solutions in candidate list likely better than current CAs")
                    break  # Candidates are sorted by time so none after will be inside
                ...
                sol_tofi, sol_uusi, sol_fi = ss.target(xx0i, xxf_fun, tof_max=(t1i-t0i), N=N_control,
                                                       time_opt=False, is_mbh=True, verbose=verbose_optim)
                ...
                if sol_fi < 1e-4:
                    ...
                    if t2i < tf:
                        ...
                        print("Solution added to candidate list")
                    else:
                        print("Solution found but not added because after final time")
                else:
            ...
            print("Feasible transfers found: {}".format(N_feas_trans))
            print("Transfers added to candidate list: {}".format(len(candidate_list) - candidate_len_before))
    ...
    print("Beam search finished. Total computation time: {} s".format(time.time() - code_start_t))
    # Save results
